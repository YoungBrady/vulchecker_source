...
data_path=$slice_root/funded
  
for type in $(ls $data_path)
do
  for cwe in $(ls $data_path/$type)
  do
    for batch in $(ls $data_path/$type/$cwe)
    do
      # if [[ $batch != "CVE-2017-9041" ]];then
      #   continue
      # fi
      data_batch="$data_path/$type/$cwe/$batch" # 源代码路径
      
      batch_dir="$slice_root/slice_all/funded/$type/$cwe/$batch" # 存放切片结果
      echo $batch_dir
      ...
      python2 $code_path/get_points.py $data_batch 2>> $error_path
      if [[ ! -s $error_path ]];then
